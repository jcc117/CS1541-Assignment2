...
// void cache_create(cache_t * put_here, int size, int blocksize, int assoc, int mem_latency)
// {
//   int i, nblocks , nsets ;
//   // struct cache_t *C = (struct cache_t *)calloc(1, sizeof(struct cache_t));
//
//   nblocks = size *1024 / blocksize ;// number of blocks in the cache
//   nsets = nblocks / assoc ;			// number of sets (entries) in the cache
//   put_here->blocksize = blocksize ;
//   put_here->nsets = nsets  ;
//   put_here->assoc = assoc;
//   put_here->mem_latency = mem_latency;
//
//   put_here->blocks= (struct cache_blk_t **)calloc(nsets, sizeof(struct cache_blk_t *));
//
//   for(i = 0; i < nsets; i++) {
// 		C->blocks[i] = (struct cache_blk_t *)calloc(assoc, sizeof(struct cache_blk_t));
// 	}
...
void updateLRU(struct cache_t *cp ,int index, int way)
{
	int k ;
